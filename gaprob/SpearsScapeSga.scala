...
class SpearsScapeSga extends jiva.ProbDefiner {
  def gaProblem = buildBooleanProb {buildr =>
    buildr name "SpearsScape"
    buildr popSize 100
    buildr chrSize 100
    buildr numEvolutions 200
    buildr fitnessFunction new SpearsSgaFitness(buildr)

    // val selector = new RouletteWheelSelector[jbool](100, buildr().rgen)
    val selector = new TournamentSelector[jbool](100, buildr.rgen, 4)
    val replacer = new GenerationalReplacer[jbool]
    val evaluator = new SgaFitnessEvaluator[jbool](buildr().ff)
...
class SpearsSgaFitness (buildr: ProbBuilder[jbool]) extends FitnessFunction[jbool] {
