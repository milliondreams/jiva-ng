...
object SchedulingProblemBuilder {
  ...
  def buildProb(chrSize:int, maxGeneValue:int,jff:JFitnessFunction[jint]) : GaProblem[jint] = {
    ...
    val ulMutOp = new RandomResetMutationOp[jint](0.05, builder.rgen)
    val mutProb:GenVarMutationProb[jint] = new GenVarMutationProb(0.05,popSize)
    val mutOp = new DetXingMutationOp(mutProb,ulMutOp)
